...
import { ObjectUtils } from "../dist/@pure-function/jsutils.js";
...
describe("ObjectUtils jsutils library", () => {
    describe("keys", () => {
        it("non empty object", () => {
            expect(ObjectUtils.keys({ h: "test" })).to.be.deep.equal(["h"]);
        });

        it("empty object", () => {
            expect(ObjectUtils.keys({})).to.be.deep.equal([]);
        });

        it("array", () => {
            expect(ObjectUtils.keys(["a", "b"])).to.be.deep.equal([]);
        });

        it("invalid object", () => {
            expect(ObjectUtils.keys(null)).to.be.deep.equal([]);
        });
    });

    describe("containsKey", () => {
        it("valid key", () => {
            expect(ObjectUtils.containsKey({ h: "test" }, "h")).to.be.true;
        });
        it("invalid key", () => {
            expect(ObjectUtils.containsKey({}, "l")).to.be.false;
        });
        it("invalid object", () => {
            expect(ObjectUtils.containsKey(undefined, "l")).to.be.false;
        });
    });

    describe("getValue", () => {
        it("valid key", () => {
            expect(ObjectUtils.getValue({ h: "test" }, "h")).to.equal("test");
        });
        it("invalid key", () => {
            expect(ObjectUtils.getValue({}, "l")).to.equal(null);
        });
        it("invalid object", () => {
            expect(ObjectUtils.getValue(undefined, "l")).to.equal(null);
        });
        it("invalid object with default value", () => {
            expect(ObjectUtils.getValue(undefined, "l", "q")).to.equal("q");
        });
    });

    describe("isPlainObjectArray", () => {
        it("valid object array", () => {
            expect(ObjectUtils.isPlainObjectArray([{ h: "test" }])).to.be.true;
        });
        ...
        it("valid nested object array without recursive flag", () => {
            expect(
                ObjectUtils.isPlainObjectArray([{ h: "test" }, [{ k: "test" }]])
            ).to.be.false;
        });
        it("object", () => {
            expect(ObjectUtils.isPlainObjectArray({ h: "test" })).to.be.false;
        });
        it("empty array", () => {
            expect(ObjectUtils.isPlainObjectArray([])).to.be.true;
        });
        it("non empty array", () => {
            expect(ObjectUtils.isPlainObjectArray(["a"])).to.be.false;
        });
    });
    ...
    describe("merge", () => {
        it("2 objects", () => {
            ...
            expect(toDict).to.be.deep.equal({ h: "test", l: "test" });
        });
        it("2 nested objects and modifications - clone deep", () => {
            ...
            ObjectUtils.merge(toDict, fromDict);
            fromDict.l["m"] = "hm";
            expect(toDict).to.be.deep.equal({ h: { k: "hk" }, l: { k: "hl" } });
        });
        it("1 object & null", () => {
            let toDict = { h: "test" };
            let fromDict = null;
            ObjectUtils.merge(toDict, fromDict);
            expect(toDict).to.be.deep.equal({ h: "test" });
        });
        it("2 objects & recursive", () => {
            ...
            expect(toDict).to.be.deep.equal({
                a: "test",
                b: { c: "test2", d: "test", e: "test" },
            });
        });
        it("2 objects, object arrays & recursive", () => {
            let toDict = {
                a: "test",
                b: { c: "test", e: [{ g: "test" }, { h: "test" }] },
            };
            let fromDict = {
                b: {
                    c: "test2",
        ...
        it("2 objects & non recursive", () => {
            ...
            expect(toDict).to.be.deep.equal({ a: "test", b: { c: "test2" } });
        });
